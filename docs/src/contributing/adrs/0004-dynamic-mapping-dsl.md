...
Good, because **Unified Transformation Standards:** Centralizing common logic into a "Core IDP Library" ensures consistency. When a transformation rule changes (e.g., a naming convention update), it is updated in one place, eliminating the need to modify hundreds of individual mapping scripts.(Maintainability / Extensibility)
