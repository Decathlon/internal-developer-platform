...
class EntityTemplateServiceTest {
    ...
    @BeforeEach
    void setUp() {
        entityTemplateService = new EntityTemplateService(
    ...
    class RelationPurgeTests {
        ...
        private EntityTemplate buildTemplate(List<RelationDefinition> relations) {
            return new EntityTemplate(TEMPLATE_ID, TEMPLATE_IDENTIFIER, "Web Service", "desc", List.of(), relations);
        }
        ...
        void shouldPurgeWhenRelationDefinitionRemoved() {
            var existingRelation = new RelationDefinition(UUID.randomUUID(), "owns", "microservice", true, false);
            ...
            verify(entityRepositoryPort).deleteRelationsByTemplateIdentifierAndRelationName(
                    ...
                    argThat((Collection<String> c) ->
                            c.size() == 1 && c.contains("owns")));
        }
    ...
    class PropertyPurgeTests {
        ...
        private EntityTemplate buildTemplate(List<PropertyDefinition> properties) {
            return new EntityTemplate(TEMPLATE_ID, TEMPLATE_IDENTIFIER, "Web Service", "desc", properties, List.of());
        }

        private PropertyDefinition prop(String name) {
            return new PropertyDefinition(UUID.randomUUID(), name, "desc", PropertyType.STRING, false, null);
        }
        ...
        void shouldMatchRemovedPropertiesCaseInsensitively() {
            var existingTemplate = buildTemplate(List.of(
                    new PropertyDefinition(UUID.randomUUID(), "Color", "desc",
                            PropertyType.STRING, false, null)));
            // Incoming uses lowercase — same logical property
            var updatedTemplate = buildTemplate(List.of(
                    new PropertyDefinition(null, "color", "desc",
                            PropertyType.STRING, false, null)));
