...
class PropertyRegexValidationServiceTest {
  ...
  @ParameterizedTest
  @ValueSource(strings = {"^[a-z0-9]+@[a-z0-9]+\\.[a-z]{2,}$", // email-like pattern
  ...
  void testSafeRegexPatternsAccepted(String safePattern) {
    assertDoesNotThrow(
        () -> propertyRegexValidationService.validateRegexPattern("field", safePattern));
  }
  ...
  @ParameterizedTest
  @ValueSource(strings = {"(a+)+", // nested quantifiers with +
