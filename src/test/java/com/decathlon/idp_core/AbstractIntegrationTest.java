...
public abstract class AbstractIntegrationTest {
  ...
  @SafeVarargs
  public final void mockApiCall(String path, HttpStatus status,
      Pair<String, String>... queryParameterList) {
    mockApiCall(GET, path, status, null, queryParameterList);
  }

  @SafeVarargs
  public final void mockApiCall(String path, Object response,
      Pair<String, String>... queryParameterList) {
  ...
  @SafeVarargs
  public final void mockApiCall(HttpMethod httpMethod, String path, HttpStatus status,
      Object response, Pair<String, String>... queryParameterList) {
    startMockServer();
    ...
    mockServerClient.when(requestDefinition)
        .respond(HttpResponse.response().withStatusCode(status.value())
  ...
  private static HttpRequest getRequestDefinition(HttpMethod httpMethod, String path,
    ...
    if (queryParameterList != null) {
      for (Pair<String, String> queryParameter : queryParameterList) {
        requestDefinition.withQueryStringParameter(queryParameter.getKey(),
  ...
  public MvcResult postBadRequestAndAssertEquals(String path, String jsonBodyfilePath,
  ...
  /// Helper method to perform a POST request and validate that it returns a
  /// BAD_REQUEST response and that the error description contains the expected
  /// text.
  ///
  /// @param path the URL path to send the POST request to
  /// @param jsonBodyfilePath the file path containing the JSON content to be sent
  /// in the request body
  /// @param errorDescription the text that should be contained in the error
  /// description
  /// @throws Exception if an error occurs during the mock MVC request execution
  public MvcResult postBadRequestAndAssertContains(String path, String jsonBodyfilePath,
      String errorDescription) throws Exception {
    return mockMvc
        .perform(
            MockMvcRequestBuilders.post(path).contentType(APPLICATION_JSON).accept(APPLICATION_JSON)
  ...
  /// description
  /// @throws Exception if an error occurs during the mock MVC request execution
  public MvcResult postConflictAndAssertContains(String path, String jsonBodyfilePath,
      String errorDescription) throws Exception {
    return mockMvc
        .perform(
            MockMvcRequestBuilders.post(path).contentType(APPLICATION_JSON).accept(APPLICATION_JSON)
