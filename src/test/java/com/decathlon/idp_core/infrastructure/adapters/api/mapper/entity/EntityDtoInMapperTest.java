...
class EntityDtoInMapperTest {
  ...
  void shouldMapCreateDtoToEntity() {
    ...
    var relation = EntityDtoInCommonFields.RelationDtoIn.builder().name("depends-on")
        .targetEntityIdentifiers(List.of("gateway", "database")).build();

    var commonFields = EntityDtoInCommonFields.builder().name("payment-service")
        .properties(properties).relations(List.of(relation)).build();

    var createDto = EntityCreateDtoIn.builder().identifier("payment-service-1")
        .entityDtoInCommonFields(commonFields).build();
  ...
  void shouldMapUpdateDtoToEntityWithNullCollections() {
    // Given
    var commonFields = EntityDtoInCommonFields.builder().name("catalog-service").properties(null)
        .relations(null).build();

    var updateDto = EntityUpdateDtoIn.builder().entityDtoInCommonFields(commonFields).build();

    // When
    Entity result = mapper.fromPutEntityDtoInToEntity(updateDto, "service-template",
        "catalog-service-42");
