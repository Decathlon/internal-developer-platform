...
class EntityTemplateMapperTest {
  ...
  class EntityTemplateMappingTests {
    ...
    void shouldMapDtoInToEntity() {
      // Given
      var propertyRules = PropertyRulesDtoIn.builder().format(PropertyFormat.URL)
          .enumValues(new String[]{}).regex("").maxLength(200).minLength(1).maxValue(0).minValue(0)
          .build();

      var propertyDefinition = PropertyDefinitionDtoIn.builder().name("applicationName")
          .description("Name of the application").type(PropertyType.STRING).required(true)
          .rules(propertyRules).build();

      var relationDefinition = RelationDefinitionDtoIn.builder().name("dependencies")
          .targetTemplateIdentifier("service").required(false).toMany(true).build();

      var commonFields = EntityTemplateDtoInCommonFields.builder().description("A service template")
          .propertiesDefinitions(List.of(propertyDefinition))
          .relationsDefinitions(List.of(relationDefinition)).build();

      var dto = EntityTemplateCreateDtoIn.builder().identifier("service-template")
          .commonFields(commonFields).build();
    ...
    void shouldMapEntityToDtoOut() {
      // Given
      var propertyRules = new PropertyRules(UUID.randomUUID(), PropertyFormat.URL, List.of(), "",
          200, 1, 0, 0);

      var propertyDefinition = new PropertyDefinition(UUID.randomUUID(), "applicationName",
          "Name of the application", PropertyType.STRING, true, propertyRules);

      var relationDefinition = new RelationDefinition(UUID.randomUUID(), "dependencies", "service",
          false, true);

      var entity = new EntityTemplate(UUID.randomUUID(), "service-template", "Service Template",
          "A service template", List.of(propertyDefinition), List.of(relationDefinition));
    ...
    void shouldMapEntityListToDtoOutList() {
      // Given
      var entity1 = new EntityTemplate(UUID.randomUUID(), "template1", "Template 1",
          "Description 1", List.of(), List.of());

      var entity2 = new EntityTemplate(UUID.randomUUID(), "template2", "Template 2",
          "Description 2", List.of(), List.of());
  ...
  class PropertyDefinitionMappingTests {
    ...
    void shouldMapPropertyDtoInToEntity() {
      // Given
      var rules = PropertyRulesDtoIn.builder().format(PropertyFormat.EMAIL)
          .enumValues(new String[]{"value1", "value2"}).regex(".*@.*").maxLength(100).minLength(5)
          .maxValue(10).minValue(1).build();

      var dto = PropertyDefinitionDtoIn.builder().name("email").description("User email address")
          .type(PropertyType.STRING).required(true).rules(rules).build();
    ...
    void shouldMapPropertyEntityToDtoOut() {
      // Given
      var rules = new PropertyRules(UUID.randomUUID(), PropertyFormat.EMAIL,
          List.of("value1", "value2"), ".*@.*", 100, 5, 10, 1);

      var entity = new PropertyDefinition(UUID.randomUUID(), "email", "User email address",
  ...
  class PropertyRulesMappingTests {
    ...
    void shouldMapRulesDtoInToEntity() {
      // Given
      var dto = PropertyRulesDtoIn.builder().format(PropertyFormat.URL)
          .enumValues(new String[]{"HTTP", "HTTPS"}).regex("^https?://.*").maxLength(500)
          .minLength(10).maxValue(100).minValue(1).build();
    ...
    void shouldNormalizeEnumValuesToUppercase() {
      ...
      var dto = PropertyRulesDtoIn.builder()
          .enumValues(new String[]{"EMAil", "postal_code", "ACTIVE"}).build();
    ...
    void shouldMapRulesEntityToDtoOut() {
      // Given
      var entity = new PropertyRules(UUID.randomUUID(), PropertyFormat.URL,
          List.of("http", "https"), "^https?://.*", 500, 10, 100, 1);
  ...
  class RelationDefinitionMappingTests {
    ...
    void shouldMapRelationDtoInToEntity() {
      // Given
      var dto = RelationDefinitionDtoIn.builder().name("parentService")
          .targetTemplateIdentifier("service").required(true).toMany(false).build();
    ...
    void shouldMapRelationEntityToDtoOut() {
      // Given
      var entity = new RelationDefinition(UUID.randomUUID(), "childServices", "service", false,
          true);
  ...
  class ListMappingTests {
    ...
    void shouldMapPropertyDtoInListToEntityList() {
      // Given
      var dto1 = PropertyDefinitionDtoIn.builder().name("prop1").description("Property 1")
          .type(PropertyType.STRING).required(true).build();

      var dto2 = PropertyDefinitionDtoIn.builder().name("prop2").description("Property 2")
          .type(PropertyType.NUMBER).required(false).build();
    ...
    void shouldMapPropertyEntityListToDtoOutList() {
      // Given
      var entity1 = new PropertyDefinition(UUID.randomUUID(), "prop1", "Property 1",
          PropertyType.STRING, true, null);

      var entity2 = new PropertyDefinition(UUID.randomUUID(), "prop2", "Property 2",
    ...
    void shouldMapRelationDtoInListToEntityList() {
      // Given
      var dto1 = RelationDefinitionDtoIn.builder().name("rel1").targetTemplateIdentifier("target1")
          .required(true).toMany(false).build();

      var dto2 = RelationDefinitionDtoIn.builder().name("rel2").targetTemplateIdentifier("target2")
          .required(false).toMany(true).build();
    ...
    void shouldMapRelationEntityListToDtoOutList() {
      // Given
      var entity1 = new RelationDefinition(UUID.randomUUID(), "rel1", "target1", true, false);

      var entity2 = new RelationDefinition(UUID.randomUUID(), "rel2", "target2", false, true);
