...
class EntityTemplateControllerTest extends AbstractIntegrationTest {
  ...
  /// **Testing rationale:** Each test ensures the API returns the expected HTTP
  /// status codes,
  ...
  class GetTemplatesPaginatedTests {
    ...
    void getTemplates_paginated_401_without_user_token() throws Exception {
      mockMvc.perform(get(ENTITY_TEMPLATE_PATH).accept(APPLICATION_JSON))
    ...
    void getTemplates_paginated_200_custom() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/entity-templates").param("page", "1").param("size", "5")
    ...
    void getTemplates_paginated_200_invalid_pagination() throws Exception {
      ...
      mockMvc.perform(get("/api/v1/entity-templates/frontend-app").accept(APPLICATION_JSON))
          .andExpect(status().isOk()).andExpect(jsonPath("$.identifier").value("frontend-app"));
    }
    ...
    void getTemplates_paginated_200_with_valid_identifier() throws Exception {
      mockMvc.perform(get("/api/v1/entity-templates").param("identifier", "web-service")
  ...
  class PostTemplateTests {
    ...
    void postTemplate_201() throws Exception {
      mockMvc
          .perform(MockMvcRequestBuilders.post(ENTITY_TEMPLATE_PATH).contentType(APPLICATION_JSON)
    ...
    /// Tests the POST /api/v1/entity-templates endpoint when the identifier field
    /// is
    /// missing.
    /// This test verifies that:
    ...
    void postTemplate_400_property_description_missing() throws Exception {
      MvcResult res = postBadRequestAndAssertEquals(ENTITY_TEMPLATE_PATH,
          ENTITY_TEMPLATE_JSON_TEST_PATH
              + "postEntityTemplate_400_property_description_missing.json",
    ...
    /// Tests the POST /api/v1/entity-templates endpoint with no property
    /// definitions.
    /// This test verifies that:
    ...
    void postTemplate_201_without_properties() throws Exception {
      mockMvc
          .perform(MockMvcRequestBuilders.post(ENTITY_TEMPLATE_PATH).contentType(APPLICATION_JSON)
              .accept(APPLICATION_JSON).with(csrf())
              .content(getJsonTestFileContent(ENTITY_TEMPLATE_JSON_TEST_PATH
                  + "postEntityTemplate_201_without_properties.json")))
          .andExpect(status().isCreated()).andReturn();
    }
    ...
    void postTemplate_201_with_empty_properties() throws Exception {
      mockMvc
          .perform(MockMvcRequestBuilders.post(ENTITY_TEMPLATE_PATH).contentType(APPLICATION_JSON)
              .accept(APPLICATION_JSON).with(csrf())
              .content(getJsonTestFileContent(ENTITY_TEMPLATE_JSON_TEST_PATH
                  + "postEntityTemplate_201_with_empty_properties.json")))
          .andExpect(status().isCreated()).andReturn();
    }

    /// Tests POST endpoint when duplicate property names are provided
    /// (case-insensitive).
    /// Verifies that PropertyNameAlreadyExistsException is thrown and returns 400
    /// Bad Request.
    @Test
    @WithMockUser()
    ...
    void postTemplate_400_duplicate_property_names() throws Exception {
      mockMvc
          .perform(MockMvcRequestBuilders.post(ENTITY_TEMPLATE_PATH).contentType(APPLICATION_JSON)
              .accept(APPLICATION_JSON).with(csrf())
              .content(getJsonTestFileContent(ENTITY_TEMPLATE_JSON_TEST_PATH
                  + "postEntityTemplate_400_duplicate_property_names.json")))
          ...
          .andExpect(jsonPath("$.status").value(400)).andExpect(jsonPath("$.detail").value(
              "Property name 'applicationname' already exists within the template. Property names must be unique."));
    }

    /// Tests POST endpoint when duplicate relation names are provided
    /// (case-insensitive).
    /// Verifies that RelationNameAlreadyExistsException is thrown and returns 400
    /// Bad Request.
    @Test
    @WithMockUser()
    ...
    void postTemplate_400_duplicate_relation_names() throws Exception {
      mockMvc
          .perform(MockMvcRequestBuilders.post(ENTITY_TEMPLATE_PATH).contentType(APPLICATION_JSON)
              .accept(APPLICATION_JSON).with(csrf())
              .content(getJsonTestFileContent(ENTITY_TEMPLATE_JSON_TEST_PATH
                  + "postEntityTemplate_400_duplicate_relation_names.json")))
          .andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request"))
          .andExpect(jsonPath("$.status").value(400)).andExpect(jsonPath("$.detail").value(
              "Relation name 'belongsto' already exists within the template. Relation names must be unique."));
    }

    /// Tests POST endpoint when relation targets non-existent template.
    /// Verifies that TargetTemplateNotFoundException is thrown and returns 400 Bad
    /// Request.
    @Test
    @WithMockUser()
    ...
    void postTemplate_400_target_template_not_found() throws Exception {
      mockMvc
          .perform(MockMvcRequestBuilders.post(ENTITY_TEMPLATE_PATH).contentType(APPLICATION_JSON)
              .accept(APPLICATION_JSON).with(csrf())
              .content(getJsonTestFileContent(ENTITY_TEMPLATE_JSON_TEST_PATH
                  + "postEntityTemplate_400_target_template_not_found.json")))
          .andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request"))
          .andExpect(jsonPath("$.status").value(400)).andExpect(jsonPath("$.detail")
              .value("Target template with identifier 'non-existent-template' does not exist."));
    }

    /// Tests POST endpoint when a relation's targetTemplateIdentifier equals the
    /// template's own identifier.
    /// Verifies that RelationSelfReferenceException is thrown and returns 400 Bad
    /// Request.
    @Test
    @WithMockUser()
    ...
    void postTemplate_400_relation_self_reference() throws Exception {
      mockMvc
          .perform(MockMvcRequestBuilders.post(ENTITY_TEMPLATE_PATH).contentType(APPLICATION_JSON)
              .accept(APPLICATION_JSON).with(csrf())
              .content(getJsonTestFileContent(ENTITY_TEMPLATE_JSON_TEST_PATH
                  + "postTemplate_400_relation_target_references_itself.json")))
          .andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request"))
          .andExpect(jsonPath("$.status").value(400))
          .andExpect(jsonPath("$.detail").value(containsString(
              "Relation 'circular' cannot reference its own template 'self-ref-template'")));
    }
  ...
  class PutTemplateTests {
    ...
    void putTemplate_without_user_token_401() throws Exception {
      ...
      mockMvc
          .perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
    ...
    void putTemplate_shouldMergePropertyRules() throws Exception {
      ...
      mockMvc
          .perform(MockMvcRequestBuilders.post(ENTITY_TEMPLATE_PATH).contentType(APPLICATION_JSON)
              .accept(APPLICATION_JSON).with(csrf())
              .content(getJsonTestFileContent(PostTemplateTests.ENTITY_TEMPLATE_JSON_TEST_PATH
                  + "postEntityTemplateWithoutRelationsDefinitions_201.json")))
          .andExpect(status().isCreated());

      EntityTemplate initialTemplate = entityTemplateRepository.findByIdentifier("temp-test-99")
          .orElseThrow();
      ...
      EntityTemplate updatedTemplate = entityTemplateRepository.findByIdentifier("temp-test-99")
          .orElseThrow();
    ...
    void putTemplate_updateRelations_200() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/template-rel-test")
          .contentType(APPLICATION_JSON).accept(APPLICATION_JSON).with(csrf()).content(updateJson))
          .andExpect(status().isOk());
      ...
      Map<String, RelationDefinition> relationsMap = updatedTemplate.relationsDefinitions().stream()
      ...
      assertThat(relationsMap.get("belongsTo").targetTemplateIdentifier())
          .isEqualTo("database-service");
    ...
    void putTemplate_200() throws Exception {
      ...
      Optional<EntityTemplate> entityTemplateUpdated = entityTemplateRepository
          .findByIdentifier("web-service");
    ...
    /// Tests the PUT /api/v1/entity-templates/{identifier} endpoint without
    /// properties.
    /// This test verifies that:
    ...
    void putTemplate_200_without_properties() throws Exception {
      ...
      mockMvc
          .perform(
              MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
                  .contentType(APPLICATION_JSON).accept(APPLICATION_JSON).with(csrf())
                  .content(getJsonTestFileContent(PostTemplateTests.ENTITY_TEMPLATE_JSON_TEST_PATH
                      + "putEntityTemplate_200_without_properties.json")))
          .andExpect(status().isOk());
    }

    /// Tests the PUT /api/v1/entity-templates/{identifier} endpoint with empty
    /// properties array.
    /// This test verifies that:
    ...
    void putTemplate_200_with_empty_properties() throws Exception {
      ...
      mockMvc
          .perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
              .contentType(APPLICATION_JSON).accept(APPLICATION_JSON).with(csrf())
              .content(getJsonTestFileContent(PostTemplateTests.ENTITY_TEMPLATE_JSON_TEST_PATH
                  + "putEntityTemplate_200_with_empty_properties.json")))
          .andExpect(status().isOk());
    }
    ...
    void putTemplate_404_withUnknownIdentifier() throws Exception {
      ...
      mockMvc
          ...
          .andExpect(jsonPath("$.status").value(404)).andExpect(
              jsonPath("$.detail").value("Template not found with identifier: unknown-identifier"));
    }
    ...
    void putTemplate_400_propertyNameIsMissing() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
          ...
          .andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request"))
          .andExpect(jsonPath("$.status").value(400))
          .andExpect(jsonPath("$.detail").value("Property name is mandatory and cannot be blank"));
    }
    ...
    void putTemplate_400_propertyDescriptionIsBlank() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
          ...
          .andExpect(jsonPath("$.status").value(400)).andExpect(
              jsonPath("$.detail").value("Property description is mandatory and cannot be blank"));
    }
    ...
    void putTemplate_400_propertyTypeIsMissing() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
          ...
          .andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request"))
          .andExpect(jsonPath("$.status").value(400))
          .andExpect(jsonPath("$.detail").value("Property type is mandatory"));
    }
    ...
    void putTemplate_409_whenIdentifierAlreadyExists() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
          ...
          .andExpect(jsonPath("$.status").value(409)).andExpect(
              jsonPath("$.detail").value("The entity template name Microservice already exists"));
    }
    ...
    void putTemplate_400_name_missing() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
          .contentType(APPLICATION_JSON).accept(APPLICATION_JSON).with(csrf())
          .content(getJsonTestFileContent(
              "integration_test/json/entity-template/v1/putEntityTemplate_400_name_missing.json")))
          .andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request"))
          .andExpect(jsonPath("$.status").value(400))
          .andExpect(jsonPath("$.detail").value(ValidationMessages.TEMPLATE_NAME_MANDATORY));
    }
    ...
    void putTemplate_400_name_blank() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
          .contentType(APPLICATION_JSON).accept(APPLICATION_JSON).with(csrf())
          .content(getJsonTestFileContent(
              "integration_test/json/entity-template/v1/putEntityTemplate_400_name_blank.json")))
          .andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request"))
          .andExpect(jsonPath("$.status").value(400)).andExpect(jsonPath("$.detail")
              .value(containsString(ValidationMessages.TEMPLATE_NAME_MANDATORY)));
    }
    ...
    void putTemplate_400_name_too_long() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
          .contentType(APPLICATION_JSON).accept(APPLICATION_JSON).with(csrf())
          .content(getJsonTestFileContent(
              "integration_test/json/entity-template/v1/putEntityTemplate_400_name_wrong_size.json")))
          .andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request"))
          .andExpect(jsonPath("$.status").value(400))
          .andExpect(jsonPath("$.detail").value(ValidationMessages.TEMPLATE_NAME_MAX_SIZE));
    }
    ...
    void putTemplate_400_name_invalid_pattern() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
          .contentType(APPLICATION_JSON).accept(APPLICATION_JSON).with(csrf())
          .content(getJsonTestFileContent(
              "integration_test/json/entity-template/v1/putEntityTemplate_400_name_invalid_pattern.json")))
          .andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request"))
          .andExpect(jsonPath("$.status").value(400))
          .andExpect(jsonPath("$.detail").value(ValidationMessages.TEMPLATE_NAME_FORMAT));
    }
    ...
    void putTemplate_400_identifier_in_body() throws Exception {
      ...
      mockMvc
          .perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
              .contentType(APPLICATION_JSON).accept(APPLICATION_JSON).with(csrf())
              .content(getJsonTestFileContent(PostTemplateTests.ENTITY_TEMPLATE_JSON_TEST_PATH
                  + "putEntityTemplate_400_identifier_in_body.json")))
          .andExpect(status().isBadRequest());
    }

    /// Tests PUT endpoint when attempting to change property type on existing
    /// property.
    /// Verifies that PropertyTypeChangeException is thrown and returns 400 Bad
    /// Request.
    @Test
    @WithMockUser()
    @DisplayName("Should return 400 when changing existing property type")
    void putTemplate_400_type_change() throws Exception {
      String identifier = "web-service";
      mockMvc
          .perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
              .contentType(APPLICATION_JSON).accept(APPLICATION_JSON).with(csrf())
      ...
      mockMvc
          .perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
              .contentType(APPLICATION_JSON).accept(APPLICATION_JSON).with(csrf())
              .content(getJsonTestFileContent(PostTemplateTests.ENTITY_TEMPLATE_JSON_TEST_PATH
                  + "putTemplate_400_type_change.json")))
          .andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request"))
          .andExpect(jsonPath("$.status").value(400)).andExpect(jsonPath("$.detail").value(
              "Cannot change type of property 'name' from STRING to NUMBER. Property types cannot be modified after creation. Please delete and recreate the property instead."));
    }

    /// Tests PUT endpoint when attempting to change targetTemplateIdentifier on an
    /// existing relation.
    /// Verifies that RelationTargetTemplateChangeException is thrown and returns
    /// 400 Bad Request.
    @Test
    @WithMockUser()
    ...
    void putTemplate_400_target_template_identifier_change() throws Exception {
      ...
      mockMvc
          .perform(MockMvcRequestBuilders.put(ENTITY_TEMPLATE_PATH + "/" + identifier)
              .contentType(APPLICATION_JSON).accept(APPLICATION_JSON).with(csrf())
              .content(getJsonTestFileContent(PostTemplateTests.ENTITY_TEMPLATE_JSON_TEST_PATH
                  + "putTemplate_400_target_template_identifier_change.json")))
          .andExpect(status().isBadRequest()).andExpect(jsonPath("$.title").value("Bad Request"))
          .andExpect(jsonPath("$.status").value(400))
          .andExpect(jsonPath("$.detail").value(containsString(
              "Cannot change target template of relation 'dependencies' from 'service' to 'service-modified'")));
    }
