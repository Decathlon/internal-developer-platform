...
class InboundWebhookMapperTest {
  ...
  private final InboundWebhookMapper mapper = new InboundWebhookMapper(
      new EntityDynamicMappingMapper());
  ...
  void shouldUsePathIdentifierForUpdateMapping() {
    var request = new InboundWebhookUpdateDtoIn("GitHub DORA", "Collect deployment events", true,
        List.of("deployment-mapping"),
    ...
    assertThat(domain.name()).isEqualTo("GitHub DORA");
