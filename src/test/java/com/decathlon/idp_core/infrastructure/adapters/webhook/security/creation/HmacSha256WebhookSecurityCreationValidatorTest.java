...
class HmacSha256WebhookSecurityCreationValidatorTest {
  ...
  @BeforeEach
  void setUp() {
    validator = new HmacSha256SecurityValidator();
  }
  ...
  class ValidConfigurations {
    ...
    void shouldPassWhenRequiredFieldsAreCamelCase() {
      Map<String, String> config = Map.of("headerName", "X-Hub-Signature-256", "secretAlias",
          "MY_GITHUB_SECRET");
      assertThatCode(() -> validator.validateConfiguration(config)).doesNotThrowAnyException();
    }

    @Test
    @DisplayName("validateConfiguration() -> Passes when secret_alias is provided as an environment reference")
    void shouldPassWhenSecretAliasUsesEnvironmentReference() {
      Map<String, String> config = Map.of("header_name", "X-Hub-Signature-256", "secret_alias",
          "${MY_GITHUB_SECRET}");
      assertThatCode(() -> validator.validateConfiguration(config)).doesNotThrowAnyException();
    }
