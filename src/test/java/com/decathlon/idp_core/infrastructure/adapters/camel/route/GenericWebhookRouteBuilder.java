...
public class GenericWebhookRouteBuilder extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        onException(WebhookConnectorNotFoundException.class)
            .handled(true)
            .log(LoggingLevel.WARN, "No webhook connector found for identifier: ${exchangeProperty.webhookId}")
        ...
        from("direct:process-generic-webhook")
            .routeId("webhook-config-lookup-pipeline")
            .log(LoggingLevel.INFO, "Received generic webhook request for identifier: ${header.webhookIdentifier}")

            // Extract the path parameter directly into a header variable
            .setProperty("webhookId", header("webhookIdentifier"))

            .log(LoggingLevel.INFO, "Fetching webhook connector configuration for identifier: ${exchangeProperty.webhookId}")
