...
-- Purpose: Support the EXISTS-based search specifications and LOWER()-based operators
--          with index scans instead of full table scans.
--
-- Strategy:
--   - Btree functional lower(col) indexes -> EQ / NEQ / STARTS_WITH comparisons using LOWER()
...
-- Functional btree indexes for EQ / NEQ / STARTS_WITH which use LOWER(col)
...
COMMENT ON INDEX idx_entity_name_lower IS 'Supports LOWER(name) comparisons for EQ, NEQ, and STARTS_WITH operators';
COMMENT ON INDEX idx_entity_identifier_lower IS 'Supports LOWER(identifier) comparisons for EQ, NEQ, and STARTS_WITH operators';
COMMENT ON INDEX idx_entity_template_identifier_lower IS 'Supports LOWER(template_identifier) comparisons for EQ, NEQ, and STARTS_WITH operators';
