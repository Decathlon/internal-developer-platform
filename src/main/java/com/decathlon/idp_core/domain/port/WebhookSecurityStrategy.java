...
public interface WebhookSecurityStrategy {
     ...
     * @throws com.decathlon.idp_core.domain.exception.webhook.WebhookSecurityConfigurationException if validation fails
     */
    void validateConfiguration(Map<String, String> config);

    /**
     * Validates an incoming webhook request against the stored security configuration.
     *
     * @param security the stored webhook security configuration
     * @param headers the HTTP request headers from the webhook
     * @param rawBody the raw request body bytes
     * @throws com.decathlon.idp_core.domain.exception.webhook.WebhookAuthenticationException if validation fails
     */
    void validateRequest(WebhookSecurity security, Map<String, String> headers, byte[] rawBody);
}
