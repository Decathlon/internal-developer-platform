...
public class EntityTemplateValidationService {
  ...
  public void validateForCreation(EntityTemplate entityTemplate) {
    ...
    if (entityTemplate.propertiesDefinitions() != null) {
      propertyDefinitionValidationService
          .validatePropertyNamesUniqueness(entityTemplate.propertiesDefinitions());
      validateTemplateProperties(entityTemplate);
    }
    if (entityTemplate.relationsDefinitions() != null) {
      relationDefinitionValidationService
          .validateRelationNamesUniqueness(entityTemplate.relationsDefinitions());
  ...
  ///
  /// **Business rules enforced:**
  /// - If the identifier changed, the new value must not collide with another
  /// template.
  /// - If the name changed, the new value must not collide with another template.
  /// - Property rules in the merged template must be compatible with their
  /// declared type.
  /// - Relation names must be unique within the template.
  /// - No relation may target the template's own identifier (no self-reference).
  /// - All target templates referenced by relations must exist in the system.
  ...
  /// @throws RelationCannotTargetItselfException when a relation targets the
  /// template itself
  public void validateForUpdate(String currentIdentifier, String existingName,
      EntityTemplate existingTemplate, EntityTemplate mergedTemplate) {
    ...
    if (mergedTemplate.propertiesDefinitions() != null) {
      propertyDefinitionValidationService
          .validatePropertyNamesUniqueness(mergedTemplate.propertiesDefinitions());
      propertyDefinitionValidationService.validateTypeChanges(
          existingTemplate.propertiesDefinitions(), mergedTemplate.propertiesDefinitions());
      validateTemplateProperties(mergedTemplate);
    }
    if (mergedTemplate.relationsDefinitions() != null) {
      relationDefinitionValidationService
          .validateRelationNamesUniqueness(mergedTemplate.relationsDefinitions());
      relationDefinitionValidationService.validateTargetTemplateChanges(
          existingTemplate.relationsDefinitions(), mergedTemplate.relationsDefinitions());
