...
public class EntityService {
  ...
  public Entity updateEntity(String templateIdentifier, String entityIdentifier,
    ...
    /// Cleans up relations in parent entities that reference the deleted entity to maintain referential integrity.
    ///
    /// **Contract:** Finds all entities that have relations targeting the deleted entity and removes those relations
    /// to prevent dangling references. This is necessary to maintain data integrity after an entity is deleted.
    ///
    /// @param entityIdentifier the identifier of the entity that was deleted, used to find and clean up related entities
    private void removedRelationRelated(final String entityIdentifier) {
        ...
        for (Entity parent : parentEntities) {
            EntityTemplate parentTemplate = entityTemplateService.getEntityTemplateByIdentifier(parent.templateIdentifier());
    ...
    /// Removes the specified entity identifier from the relations of the parent entity, ensuring that required single-target relations are not left empty.
    ///
    /// **Contract:** Iterates through the relations of the parent entity, removing the target identifier from any relation
    /// that contains it. If a relation becomes empty as a result, checks the relation definition to determine if it is required and single-target;
    ///  if so, the relation is not removed to avoid leaving the parent entity in an invalid state.
    /// @param parent the entity whose relations are being cleaned up
    /// @param parentTemplate the template of the parent entity, used to check relation definitions
    /// @param entityIdentifierToRemove the identifier of the entity being deleted, which should be removed from the relations of the parent entity
    /// @return a new Entity instance with updated relations, reflecting the removal of the specified entity identifier
    /// **Note:** This method assumes that the parent entity and its template are valid and exist, as it is called in the context of cleaning up after a known entity deletion.
    /// It focuses solely on relation cleanup and does not perform additional validations or checks beyond what is necessary for maintaining referential integrity.
    private Entity cleanUpRelations(final Entity parent, final EntityTemplate parentTemplate, final String entityIdentifierToRemove) {
        ...
        for (Relation relation : currentRelations) {
            ...
            List<String> updatedTargets = currentTargets.stream()
                    .filter(target -> !entityIdentifierToRemove.equals(target))
                    .toList();
            ...
            updatedRelations.add(new Relation(
                    relation.id(),
                    relation.name(),
                    relation.targetTemplateIdentifier(),
                    updatedTargets));
        }
    ...
    private RelationDefinition getRelationDefinition(final EntityTemplate template, final String relationName) {
        if (template.relationsDefinitions() == null) {
            return null;
        }
        return template.relationsDefinitions().stream()
                .filter(definition -> relationName.equals(definition.name()))
                .findFirst()
                .orElse(null);
    }

    /// Validates that an entity with the specified template and identifier exists, throwing an exception if not found.
    ///
    /// **Contract:** Checks the existence of the entity using the repository. If the entity is
    /// not found, throws an EntityNotFoundException with the relevant template and entity identifiers for error reporting.
    /// @param templateIdentifier the identifier of the template to which the entity belongs, used for lookup and error reporting
    /// @param entityIdentifier the unique identifier of the entity within the template, used for lookup and error reporting
    /// @throws EntityNotFoundException if no entity matching the template and entity identifiers is found in the repository, indicating that the entity does not exist
    /// @return the Entity instance that matches the specified template and entity identifiers, if found.
