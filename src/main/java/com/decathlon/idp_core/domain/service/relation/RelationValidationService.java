...
public class RelationValidationService {
    ...
    /// Validates that all target entity identifiers in the relation actually exist in the database.
    ///
    /// @param relation   the relation whose target entity identifiers are to be validated
    /// @param violations the accumulator for any validation violations found during the process
    ...
    /// Extracts non-null, non-blank target entity identifiers from the relation, returning an empty list if the relation or its target identifiers are null.
    ///
    /// @param relation the relation from which to extract target entity identifiers
    /// @return a list of valid target entity identifiers; empty if none are valid or if the relation is null
