...
public class SearchFilterParser {
  ...
  /// Parses and validates a [RawSearchFilterNode] tree into a typed
  /// [SearchFilterNode] tree.
  ///
  /// @param raw the root of the raw filter tree; null is treated as "no filter"
  /// and returns an empty AND group
  /// @return the validated, type-safe domain filter tree
  /// @throws InvalidSearchQueryException when the raw tree contains structural
  /// errors or exceeds safety limits
  ...
  private SearchFilterNode.Group convertGroup(RawSearchFilterNode.Group raw, int depth,
      int[] criteriaCounter) {
    ...
    List<SearchFilterNode> children = raw.nodes().stream()
        .map(child -> convertNode(child, depth + 1, criteriaCounter)).toList();

    return new SearchFilterNode.Group(connector, children);
  }

  private SearchFilterNode.Criterion convertCriterion(RawSearchFilterNode.Criterion raw,
      int[] criteriaCounter) {
    ...
    if (criteriaCounter[0] > SearchConstraints.MAX_TOTAL_CRITERIA) {
      throw new InvalidSearchQueryException(ValidationMessages.SEARCH_TOO_MANY_CRITERIA
          .formatted(SearchConstraints.MAX_TOTAL_CRITERIA));
    }
