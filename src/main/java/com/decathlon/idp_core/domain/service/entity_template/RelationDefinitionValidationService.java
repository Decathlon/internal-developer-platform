...
public class RelationDefinitionValidationService {
  ...
  public void validateRelationNamesUniqueness(List<RelationDefinition> relations) {
    ...
    relations.stream().map(RelationDefinition::name).filter(Objects::nonNull)
        .map(name -> name.toLowerCase(Locale.ROOT)).filter(name -> !names.add(name)).findFirst()
        .ifPresent(name -> {
  ...
  public void validateTargetTemplatesExist(List<RelationDefinition> relations) {
    for (RelationDefinition relation : relations) {
      ...
      if (targetIdentifier == null
          || !entityTemplateRepositoryPort.existsByIdentifier(targetIdentifier)) {
        throw new TargetTemplateNotFoundException(targetIdentifier);
      }
    }
  }

  /// Validates that no relation definition targets itself (the template that owns
  /// it).
  ///
  /// @param templateIdentifier the identifier of the template being created or
  /// updated
  /// @param relations the list of relation definitions to check
  /// @throws RelationCannotTargetItselfException if any relation's target
  /// template equals the template's own identifier
  public void validateRelationNoSelfReference(String templateIdentifier,
      List<RelationDefinition> relations) {
    if (templateIdentifier == null || relations == null || relations.isEmpty()) {
      return;
    }
    relations.stream().filter(r -> templateIdentifier.equals(r.targetTemplateIdentifier()))
        .findFirst().ifPresent(r -> {
          throw new RelationCannotTargetItselfException(r.name(), templateIdentifier);
        });
  }

  /// Validates that target template identifiers are not changed on existing
  /// relations.
  ///
  /// **Contract:** Enforces the invariant that relation target templates cannot
  /// be
  /// modified after initial creation. Any attempt to change a target template
  /// identifier
  /// is forbidden, as existing entity relation values would point to the wrong
  /// template type.
  ///
  /// @param existingRelations the existing relation definitions (from the
  /// persisted template)
  /// @param incomingRelations the new/updated relation definitions
  /// @throws RelationTargetTemplateChangeException if any relation target
  /// template change is attempted
  public void validateTargetTemplateChanges(List<RelationDefinition> existingRelations,
      List<RelationDefinition> incomingRelations) {
    if (existingRelations == null || existingRelations.isEmpty() || incomingRelations == null
    ...
    for (RelationDefinition existing : existingRelations) {
      ...
      boolean targetChanged = incoming != null && !Objects
          .equals(existing.targetTemplateIdentifier(), incoming.targetTemplateIdentifier());

      if (targetChanged) {
        throw new RelationTargetTemplateChangeException(existing.name(),
            existing.targetTemplateIdentifier(), incoming.targetTemplateIdentifier());
      }
    }
  }
