...
public class PropertyRegexValidationService {
  ...
  private static final ExecutorService VALIDATION_EXECUTOR = Executors
  ...
  // Validation ReDoS probe string designed to trigger backtracking in vulnerable
  // patterns
  ...
  /// Validates pattern matching with a timeout to detect ReDoS (Regular
  /// Expression Denial of Service) vulnerabilities.
  ///
  /// Executes a pattern match against a stress probe within a 10 ms timeout using
  /// a shared, bounded executor
  /// If the pattern takes longer than the timeout, it is rejected as potentially
  /// vulnerable to catastrophic backtracking.
  ///
  /// @param propertyName name of the property (for error reporting)
  /// @param pattern the compiled pattern to test
  /// @throws PropertyDefinitionRulesConflictException if the pattern times out or
  /// validation fails
  private void validatePatternWithTimeout(String propertyName, Pattern pattern) {
    Future<Boolean> future = VALIDATION_EXECUTOR
        .submit(() -> pattern.matcher(STRESS_PROBE).matches());
    try {
  ...
  private boolean containsDangerousPatterns(String pattern) {
    return hasNestedQuantifiers(pattern) || hasQuantifiedAlternation(pattern)
        || hasLargeRepetitionBounds(pattern) || hasLookaroundsWithQuantifiers(pattern);
  }
  ...
  private boolean hasNestedQuantifiers(String pattern) {
    for (int i = 0; i < pattern.length(); i++) {
      if (pattern.charAt(i) == '('
          && matchesQuantifiedGroup(pattern, i, this::containsQuantifier)) {
        return true;
      }
    }
    return false;
  ...
  /// Checks if a group starting at index i matches the quantified pattern
  /// criteria.
  ...
  private boolean matchesQuantifiedGroup(String pattern, int groupStartIndex,
  ...
  private boolean hasQuantifiedAlternation(String pattern) {
    for (int i = 0; i < pattern.length(); i++) {
      if (pattern.charAt(i) == '('
          && matchesQuantifiedGroup(pattern, i, groupContent -> groupContent.contains("|"))) {
        return true;
      }
    }
    return false;
  ...
  /// Detects repetition bounds with excessively large upper limits like
  /// `{5,9999}`.
  /// Uses simple character-by-character analysis without regex.
  ///
  /// @param pattern the regex pattern string
  ...
  /// Checks if the repetition bound starting at position i exceeds the safe
  /// limit.
  ///
  /// @param pattern the regex pattern string
  ...
  /// Checks if the character at position i is one of the expected lookaround
  /// markers.
  ///
  /// @param pattern the regex pattern string
  /// @param i the position to check
  ...
  /// Helper: Finds the matching closing parenthesis for an opening paren at index
  /// startIndex.
  /// Handles nested parentheses correctly. Returns -1 if no matching close paren
  /// exists.
  ///
