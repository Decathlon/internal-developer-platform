...
public class EntityValidationService {
  ...
  /// (typically via
  /// [com.decathlon.idp_core.domain.port.EntityTemplateRepositoryPort])
  ...
  private void validateAgainstTemplate(EntityTemplate template, List<Property> properties) {
    Violations violations = new Violations();
    List<PropertyDefinition> definitions = Optional.ofNullable(template.propertiesDefinitions())
        .orElse(List.of());
    Map<String, Property> propertiesByName = Optional.ofNullable(properties).orElse(List.of())
        .stream().filter(p -> p.name() != null)
    ...
    for (PropertyDefinition definition : definitions) {
      ...
      propertyValidationService.validatePropertyValue(definition, property.value())
