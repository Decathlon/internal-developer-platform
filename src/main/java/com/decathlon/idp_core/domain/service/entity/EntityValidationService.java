...
/// 2. Syntactic checks on the entity itself (name/identifier, nested properties,
///    relations).
...
public class EntityValidationService {
  ...
  ///
  /// **Contract:** the caller is responsible for resolving the
  /// [EntityTemplate] (typically via
  /// [com.decathlon.idp_core.domain.port.EntityTemplateRepositoryPort]) and
  /// passing it in. This avoids a redundant database round-trip and clarifies the
  /// dependency graph of the validation service.
  ///
  ...
  /// values to check
  /// @throws EntityValidationException if any property validation rules are
  /// violated, including missing
  /// required properties
  private void validateAgainstTemplate(EntityTemplate template, Entity entity) {
    ...
    relationValidationService.validateRelationsAgainstTemplate(template, entity.relations(),
        violations);
  ...
  private void validateUniqueness(final Entity entity) {
    if (entity.identifier() != null && entityRepository
