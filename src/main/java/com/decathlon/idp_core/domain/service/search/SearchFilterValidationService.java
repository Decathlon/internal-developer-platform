...
public class SearchFilterValidationService {
    ...
    private static final Set<SearchOperator> NUMERIC_OPERATORS =
            Set.of(SearchOperator.GT, SearchOperator.GTE, SearchOperator.LT, SearchOperator.LTE);
    private static final Set<String> SIMPLE_FIELDS =
            Set.of("template", "identifier", "name", "relation", "relations_as_target");
    ...
    private void checkPropertyTypes(Set<String> numericPropertyNames, EntityTemplate template) {
        template.propertiesDefinitions().stream()
                ...
                .ifPresent(pd -> {
                    throw new InvalidSearchQueryException(
                            ValidationMessages.SEARCH_NUMERIC_OPERATOR_PROPERTY_TYPE_MISMATCH
                                    .formatted(pd.name(), template.identifier(), pd.type()));
                });
    }
