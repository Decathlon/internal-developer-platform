...
public class EntityFilterDslParser {
    ...
    /// @throws InvalidFilterDslException when the query string is malformed or exceeds safety limits
    public EntityFilter parse(String query) {
        ...
        if (criteria.size() > FilterConstraints.MAX_CRITERIA_COUNT) {
            throw new InvalidFilterDslException(
                    ValidationMessages.FILTER_TOO_MANY_CRITERIA.formatted(FilterConstraints.MAX_CRITERIA_COUNT));
        }
    ...
    private void validateOperatorCompatibility(FilterKeyType keyType, FilterOperator operator, String rawKey) {
        if (COMPARISON_INCOMPATIBLE_TYPES.contains(keyType) &&
            ...
            throw new InvalidFilterDslException(ValidationMessages.FILTER_TYPE_MISMATCH.formatted(opSymbol, rawKey));
        }
    }
    ...
    ///
    /// This is a semantic check that requires the template to be available (i.e., it
    ...
    /// @throws InvalidFilterDslException when a comparison operator is used on a non-NUMBER property
    public void validateFilterPropertyTypes(EntityFilter filter, EntityTemplate template) {
        filter.criteria().stream()
                .filter(c -> c.keyType() == FilterKeyType.PROPERTY)
                .filter(c -> c.operator() == FilterOperator.LESS_THAN || c.operator() == FilterOperator.GREATER_THAN)
                .forEach(c -> {
                    var propertyDef = template.propertiesDefinitions().stream()
                            .filter(p -> p.name().equals(c.key()))
                            .findFirst();
    ...
    private void validateLength(String rawKey, String value, String token) {
        if (rawKey.length() > FilterConstraints.MAX_KEY_VALUE_LENGTH) {
            throw new InvalidFilterDslException(
                    ValidationMessages.FILTER_KEY_TOO_LONG.formatted(FilterConstraints.MAX_KEY_VALUE_LENGTH, token));
        }
        if (value.length() > FilterConstraints.MAX_KEY_VALUE_LENGTH) {
            throw new InvalidFilterDslException(
                    ValidationMessages.FILTER_VALUE_TOO_LONG.formatted(FilterConstraints.MAX_KEY_VALUE_LENGTH, token));
        }
    }
}
