...
public class EntityDtoInMapper {
  ...
  public Entity fromEntityDtoInToEntity(EntityDtoIn entityDtoIn, String entityTemplateIdentifier) {
    ...
    List<Property> properties = entityDtoIn.getProperties() == null
        ? Collections.emptyList()
        : entityDtoIn.getProperties().entrySet().stream()
            .map((Map.Entry<String, String> entry) -> new Property(null, entry.getKey(),
                entry.getValue()))
            .toList();

    List<Relation> relations = entityDtoIn.getRelations() == null
        ? Collections.emptyList()
        : entityDtoIn.getRelations().stream().map(relDto -> new Relation(null, relDto.getName(),
            null, relDto.getTargetEntityIdentifiers())).toList();

    return new Entity(null, entityTemplateIdentifier, entityDtoIn.getName(),
        entityDtoIn.getIdentifier(), properties, relations);
  }
}
