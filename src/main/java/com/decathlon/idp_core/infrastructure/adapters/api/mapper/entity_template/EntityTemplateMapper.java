...
public class EntityTemplateMapper {
  ...
  /// This method maps all fields from the input DTO to create a new
  /// EntityTemplate
  /// domain entity.
  /// Nested collections (properties and relations) are recursively converted
  /// using
  /// their
  ...
  public EntityTemplate fromDtoToEntityTemplate(EntityTemplateCreateDtoIn dto) {
    ...
    return new EntityTemplate(null, dto.getIdentifier(), dto.getCommonFields().getName(),
  ...
  /// @param dto the input DTO to convert, may be null
  /// @return the converted EntityTemplate domain entity, or null if input is null
  public EntityTemplate fromPutDtoToEntityTemplate(String identifier,
      EntityTemplateUpdateDtoIn dto) {
    if (dto == null || dto.getCommonFields() == null) {
      return null;
    }

    return new EntityTemplate(null, identifier, dto.getCommonFields().getName(),
        dto.getCommonFields().getDescription(),
        toPropertyDefinitionEntities(dto.getCommonFields().getPropertiesDefinitions()),
        toRelationDefinitionEntities(dto.getCommonFields().getRelationsDefinitions()));
  }
  ...
  /// This method maps all fields from the domain entity to create a new output
  /// DTO
  /// for API responses.
  ...
  public EntityTemplateDtoOut fromEntityTemplatetoDto(EntityTemplate entity) {
    ...
    return EntityTemplateDtoOut.builder().identifier(entity.identifier()).name(entity.name())
        .description(entity.description())
        .propertiesDefinitions(toPropertyDefinitionDtos(entity.propertiesDefinitions()))
        .relationsDefinitions(toRelationDefinitionDtos(entity.relationsDefinitions())).build();
  }
  ...
  /// @return a list of converted EntityTemplateDtoOut objects, empty list if
  /// input
  /// is null
  public List<EntityTemplateDtoOut> fromEntityTemplatesToDtos(List<EntityTemplate> entities) {
    ...
    return entities.stream().map(this::fromEntityTemplatetoDto).toList();
  }
  ...
  public PropertyDefinition toToPropertyDefinition(PropertyDefinitionDtoIn dto) {
    ...
    return new PropertyDefinition(null, dto.getName(), dto.getDescription(), dto.getType(),
        dto.isRequired(), toPropertyRules(dto.getRules()));
  }
  ...
  public PropertyDefinitionDtoOut toDto(PropertyDefinition entity) {
    ...
    return PropertyDefinitionDtoOut.builder().name(entity.name()).description(entity.description())
        .type(entity.type()).required(entity.required()).rules(toDto(entity.rules())).build();
  }
  ...
  public List<PropertyDefinition> toPropertyDefinitionEntities(List<PropertyDefinitionDtoIn> dtos) {
    ...
    return dtos.stream().map(this::toToPropertyDefinition).toList();
  }

  public List<PropertyDefinitionDtoOut> toPropertyDefinitionDtos(
      List<PropertyDefinition> entities) {
    if (entities == null) {
      return List.of();
    }
  ...
  public PropertyRules toPropertyRules(PropertyRulesDtoIn dto) {
    ...
    return new PropertyRules(null, dto.getFormat(),
        dto.getEnumValues() != null
            ? List.of(dto.getEnumValues()).stream().map(String::toUpperCase).toList()
            : null,
        dto.getRegex(), dto.getMaxLength(), dto.getMinLength(), dto.getMaxValue(),
        dto.getMinValue());
  }
  ...
  public PropertyRulesDtoOut toDto(PropertyRules entity) {
    ...
    return PropertyRulesDtoOut.builder().format(entity.format())
        .enumValues(entity.enumValues() != null ? entity.enumValues().toArray(new String[0]) : null)
        .regex(entity.regex()).maxLength(entity.maxLength()).minLength(entity.minLength())
        .maxValue(entity.maxValue()).minValue(entity.minValue()).build();
  }
  ...
  public RelationDefinition toRelationDefinition(RelationDefinitionDtoIn dto) {
    ...
    return new RelationDefinition(null, dto.getName(), dto.getTargetTemplateIdentifier(),
        dto.isRequired(), dto.isToMany());
  }
  ...
  public RelationDefinitionDtoOut toDto(RelationDefinition entity) {
    ...
    return RelationDefinitionDtoOut.builder().name(entity.name())
        .targetTemplateIdentifier(entity.targetTemplateIdentifier()).required(entity.required())
        .toMany(entity.toMany()).build();
  }
  ...
  public List<RelationDefinition> toRelationDefinitionEntities(List<RelationDefinitionDtoIn> dtos) {
    ...
    return dtos.stream().map(this::toRelationDefinition).toList();
  }

  public List<RelationDefinitionDtoOut> toRelationDefinitionDtos(
      List<RelationDefinition> entities) {
    if (entities == null) {
      return List.of();
    }
    return entities.stream().map(this::toDto).toList();
