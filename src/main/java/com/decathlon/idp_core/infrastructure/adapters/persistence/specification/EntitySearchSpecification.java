...
public final class EntitySearchSpecification {
    ...
    public static Specification<EntityJpaEntity> globalTextSearch(String query) {
        ...
        Specification<EntityJpaEntity> byIdentifier =
                (root, q, cb) -> ((HibernateCriteriaBuilder) cb).ilike(root.get(IDENTIFIER), pattern, JpaPredicateBuilder.LIKE_ESCAPE_CHAR);

        Specification<EntityJpaEntity> byName =
                (root, q, cb) -> ((HibernateCriteriaBuilder) cb).ilike(root.get(NAME), pattern, JpaPredicateBuilder.LIKE_ESCAPE_CHAR);

        Specification<EntityJpaEntity> byTemplate =
                (root, q, cb) -> ((HibernateCriteriaBuilder) cb).ilike(root.get(TEMPLATE_IDENTIFIER), pattern, JpaPredicateBuilder.LIKE_ESCAPE_CHAR);
        ...
        Specification<EntityJpaEntity> byAnyProperty = (root, queryCtx, cb) -> {
            ...
            sub.select(cb.literal(1))
                    .where(
                            ...
                            ((HibernateCriteriaBuilder) cb).ilike(propJoin.get("value").as(String.class), pattern, JpaPredicateBuilder.LIKE_ESCAPE_CHAR)
                    );
