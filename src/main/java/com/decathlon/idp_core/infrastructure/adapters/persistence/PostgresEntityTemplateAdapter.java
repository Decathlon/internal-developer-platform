...
public class PostgresEntityTemplateAdapter implements EntityTemplateRepositoryPort {
  ...
  private void mergePropertyDefinitions(EntityTemplateJpaEntity jpa, EntityTemplate domain) {
    ...
    Set<String> updatedNames = domain.propertiesDefinitions().stream().map(p -> p.name())
    ...
    for (var domProp : domain.propertiesDefinitions()) {
      ...
      if (ex != null) {
        ...
        PropertyDefinitionJpaEntity newProp = PropertyDefinitionJpaEntity.builder().id(domProp.id())
            .name(domProp.name()).description(domProp.description()).type(domProp.type())
            .required(domProp.required())
            .rules(domProp.rules() != null ? toRulesJpa(domProp.rules()) : null).build();
  ...
  private void mergeRules(PropertyDefinitionJpaEntity jpaProp,
    ...
    if (ex != null) {
      ...
      ex.setEnumValues(
          domRules.enumValues() != null ? domRules.enumValues().toArray(new String[0]) : null);
  ...
  private PropertyRulesJpaEntity toRulesJpa(
      com.decathlon.idp_core.domain.model.entity_template.PropertyRules d) {
    return PropertyRulesJpaEntity.builder().id(d.id()).format(d.format())
        .enumValues(d.enumValues() != null ? d.enumValues().toArray(new String[0]) : null)
        .regex(d.regex()).maxLength(d.maxLength()).minLength(d.minLength()).maxValue(d.maxValue())
        .minValue(d.minValue()).build();
  }
  ...
  private void mergeRelationDefinitions(EntityTemplateJpaEntity jpa, EntityTemplate domain) {
    ...
    Set<String> updatedNames = domain.relationsDefinitions().stream().map(r -> r.name())
        .collect(Collectors.toSet());
    ...
    for (var domRel : domain.relationsDefinitions()) {
      ...
      if (ex != null) {
        ...
        RelationDefinitionJpaEntity newRel = RelationDefinitionJpaEntity.builder()
            .name(domRel.name()).targetTemplateIdentifier(domRel.targetTemplateIdentifier())
            .required(domRel.required()).toMany(domRel.toMany()).build();
        existing.add(newRel);
      }
    }
