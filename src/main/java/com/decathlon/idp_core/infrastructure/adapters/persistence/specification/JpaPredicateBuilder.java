...
final class JpaPredicateBuilder {
    ...
    /// Builds a [Predicate] for the given [SearchOperator] against the provided field expression.
    ///
    /// - EQ / NEQ use `LOWER(field) = LOWER(value)` to leverage functional btree indexes.
    /// - CONTAINS / NOT_CONTAINS / STARTS_WITH / ENDS_WITH use `ILIKE` to leverage GIN
    ///   trigram indexes and avoid redundant client-side lowercasing.
    /// - GT / GTE / LT / LTE cast the field to `NUMERIC` before comparing.
    ///
    /// @param cb       the JPA criteria builder (must be a [HibernateCriteriaBuilder] at runtime)
    ...
    static Predicate buildNumericPredicate(
            CriteriaBuilder cb,
            Expression<?> field,
            SearchOperator operator,
            BigDecimal numericValue) {
        // Explicit SQL CAST(field AS NUMERIC): the property value column is VARCHAR; without
    ...
    /// Escapes SQL LIKE wildcards (`%` and `_`) in the given value so they are treated as
    /// literal characters rather than pattern metacharacters.
    ///
    /// Used by all ILIKE-based operators. The value does not need to be pre-lowercased
