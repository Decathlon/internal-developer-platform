...
public interface JpaEntityRepository extends JpaRepository<EntityJpaEntity, UUID> {
  ...
  /// This is called after findAllByIdentifierInWithRelations to complete the
  /// entity graph.
  ...
  /// The filter is applied inside both the outbound and inbound recursive CTE
  /// steps so that only
  /// entities reachable through the specified relations are returned, keeping the
  /// result set lean.
  @Query(value = """
      WITH RECURSIVE
      outbound_graph(identifier, template_identifier, depth) AS (
